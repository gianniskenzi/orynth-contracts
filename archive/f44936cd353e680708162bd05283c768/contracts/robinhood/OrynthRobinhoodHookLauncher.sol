...
import {OrynthEthFeeHook} from "./OrynthEthFeeHook.sol";

/// @notice Parallel Orynth launcher for native-ETH hook fees. The original launcher is unchanged.
contract OrynthRobinhoodHookLauncher is EIP712 {
    using ECDSA for bytes32;
    using LaunchPlanner for LaunchPlan;
    using PoolIdLibrary for PoolKey;

    bytes32 private constant LAUNCH_TYPEHASH = keccak256(
        "Launch(bytes32 projectId,address creator,bytes32 nameHash,bytes32 symbolHash,bytes32 metadataHash,uint256 devBuyWei,uint256 minTokensOut,uint256 nonce,uint256 deadline)"
    );

    uint24 public constant POOL_FEE = 0;
    int24 public constant TICK_SPACING = 200;
    int24 public constant INITIAL_TICK = 120_200;
    ...
    OrynthEthFeeHook public immutable feeHook;
    ...
    constructor(
        ...
        OrynthEthFeeHook feeHook_,
        address launchAuthority_,
        address platformRecipient_,
        address operator_
    ) EIP712("Orynth Robinhood Hook Launcher", "1") {
        require(
        ...
        require(address(feeHook_.poolManager()) == address(poolManager_), "hook manager mismatch");
        ...
        returns (address tokenAddress, bytes32 poolId, address feeLocker, uint256 positionTokenId)
    {
        ...
        OrynthLaunchToken token = new OrynthLaunchToken(
            request.name, request.symbol, request.metadataURI, address(this)
        );
        ...
        feeHook.registerPool(key, request.creator);
